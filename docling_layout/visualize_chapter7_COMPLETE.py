#!/usr/bin/env python3
"""
Create annotated PDF with ALL bounding boxes from Chapter 7 JSON
Uses layout_WITH_ZONA_FIX.json (with Zona classification fix applied)
"""
import json
import fitz  # PyMuPDF
...
# Paths - using clean extraction with Zona classification fix
json_path = Path(__file__).parent / "capitulo_07/outputs/layout_clean_zona_fixed.json"
...
output_pdf = Path(__file__).parent / "capitulo_07/outputs/chapter7_COMPLETE_CLEAN.pdf"

print("\n" + "█" * 80)
print("🎨 CREATING COMPLETE ANNOTATED PDF FOR CHAPTER 7")
print("█" * 80)
...
for element in elements:
    page_num = element.get('page')  # Changed from 'page_number' to match Chapter 6
    ...
    # Page numbers in JSON are 1-based (page 1 in JSON = page 0 in PDF)
    ...
    # Draw rectangle (same style as Chapter 6)
...
print("=" * 80)
print("✅ COMPLETE ANNOTATED PDF CREATED")
print("=" * 80)
print()
print(f"📁 Output file:")
print(f"   {output_pdf}")
...
print("   🟢 Green  = table")
print("   🔵🟢 Cyan   = list_item (including Zona items fixed by patch)")
print("   🟣 Magenta = picture")
print("   🟠 Orange = formula")
print()
print("📄 Open the PDF to see ALL bounding boxes with Zona fix applied!")
print("=" * 80)
