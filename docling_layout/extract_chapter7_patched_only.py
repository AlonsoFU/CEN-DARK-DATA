#!/usr/bin/env python3
"""
Chapter 7: Run ONLY the patched extraction + visualization + report
(Same methodology as Chapter 6)
"""
import json
import sys
...
print("=" * 80)
print("🎯 CHAPTER 7: Patched Extraction + Visualization + Report")
print("=" * 80)

# Load native extraction (use the lightweight one as baseline)
native_json = OUTPUT_DIR / "layout_lightweight.json"
with open(native_json) as f:
    native_data = json.load(f)
    # Convert from Docling native format to our element format
    native_elements = []
    for item in native_data.get('main-text', []):
        native_elements.append({
            'type': item.get('label', 'unknown'),
            'text': item.get('text', ''),
            'page': item.get('prov', [{}])[0].get('page', 0) if item.get('prov') else 0,
            'bbox': None  # Will extract from prov if needed
        })
...
# Draw ALL patched boxes (thin)
patched_boxes = 0
for element in patched_elements:
...
print(f"   🔵 Drew {patched_boxes} patched boxes (thin)")

# Draw patch additions ONLY (THICK RED)
patch_boxes = 0
...
page.insert_text(fitz.Point(legend_x, y_offset), "Patched (thin):", fontsize=8, color=(0, 0, 0))
y_offset += 12

patched_counts = {}
for e in patched_elements:
    t = e['type']
    patched_counts[t] = patched_counts.get(t, 0) + 1

for elem_type, count in sorted(patched_counts.items(), key=lambda x: x[1], reverse=True)[:5]:
...
report = f"""# Chapter 7 Extraction Comparison Report

**Generated**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
**Chapter**: 7 - Análisis de las causas de la falla
**Pages**: 266-347 (82 pages)
...
1. `layout_lightweight.json` - Native Docling ({len(native_elements)} elements)
2. `layout_patched.json` - With EAF Patch ({len(patched_elements)} elements)
3. `annotated_capitulo_07_COMPLETE.pdf` - Visualization (thin=all, thick red=patch additions)
