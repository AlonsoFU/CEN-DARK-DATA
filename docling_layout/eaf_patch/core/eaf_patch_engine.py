...
def _patched_process_regular_clusters(self):
    ...
    # ========================================================================
    # STEP 5: Detect Missing Titles (CHECK ALL PDF LINES, not just missing!)
    # ========================================================================
    # CRITICAL FIX: Docling often merges title lines into paragraphs
    # Example: "6. Normalización del servicio" gets merged with next paragraph
    # Solution: Check ALL PDF lines for title patterns, force-create clusters
    # even if Docling has coverage, because that coverage might be merged/lost
    # ========================================================================
    title_detector = EAFTitleDetector()
    missing_titles = []

    # Check ALL PDF lines for title patterns (not just missing ones!)
    for pdf_line in all_pdf_lines:
        text = pdf_line['text']
        bbox = pdf_line['bbox']
        page = self.page.page_no
    ...
    for i, title_block in enumerate(missing_titles):
        ...
        # ========== CHECK FOR DUPLICATE: Skip if Docling already has this title ==========
        ...
        if not is_main_chapter_title:  # Only check duplicates for non-chapter titles
            for cluster in self.regular_clusters:
                if cluster.label == DocItemLabel.SECTION_HEADER:
                    ...
                    if overlap > 0.5:
                        print(f"   ⚠️  [PATCH] Skipping duplicate title (Docling already has it):")
    ...
    if len(list_items) > 0 and _LAST_PAGE_LAST_CLUSTER is not None:
        ...
        if hasattr(_LAST_PAGE_LAST_CLUSTER, 'label') and _LAST_PAGE_LAST_CLUSTER.label == DocItemLabel.LIST_ITEM:
            # First item on THIS page connects to last item on PREVIOUS page
            first_item_connects_to_prev_page = True
            print(f"🔗 [PATCH] Cross-page connection: Previous page ended with list-item, this page starts with list-item")

    if len(list_items) > 0:
        # Step 2: Determine which are isolated vs sequential (within 3 positions on this page)
        for i, item in enumerate(list_items):
            # Check next neighbor (within this page)
            ...
            if i == 0:
                ...
                has_prev_neighbor = (item['index'] - list_items[i - 1]['index'] <= 3)
...
# Imported from core/post_processors/ module
# Note: Isolated list-item fix moved to monkey patch (page-level)

from .post_processors import apply_zona_fix_to_document
